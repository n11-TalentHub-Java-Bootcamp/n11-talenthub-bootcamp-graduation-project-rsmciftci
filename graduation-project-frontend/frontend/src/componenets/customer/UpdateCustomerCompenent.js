...
class UpdateCustomerComponent extends Component {
  ...
  submitCustomer = event =>{
      ...
      axios.put("http://localhost:8080/api/customers/",customer).then(response =>{
  ...
  customerChange(event){
    ...
    render() {
      const {id, turkishIdentityNumber, name, middleName, surname, phoneNumber, email, dateOfBirth,  monthlySalary      
      } = this.state;


        return (
          <Card className={"border border-dark bg-light text-white"}>
                  <Card.Header className='text-black'>UPDATE A CUSTOMER</Card.Header>
